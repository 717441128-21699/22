...
export default function Alerts() {
  ...
  const mergedAlerts = useMemo(() => {
    ...
    realtimeAlerts.forEach((a) => map.set(a.id, a));
  ...
  }, [mergedAlerts, level, type, status, searchKeyword]);

  const stats = useMemo(() => {
    return {
      total: mergedAlerts.length,
      level1: mergedAlerts.filter((a) => a.level === 1).length,
      level2: mergedAlerts.filter((a) => a.level === 2).length,
      resolved: mergedAlerts.filter((a) => a.status === 'resolved').length,
    };
  }, [mergedAlerts]);
  ...
  const currentLevelLabel = levelOptions.find((o) => o.value === level)?.label || '全部级别';
  const currentTypeLabel = typeOptions.find((o) => o.value === type)?.label || '全部类型';
  const currentStatusLabel = statusOptions.find((o) => o.value === status)?.label || '全部状态';

  if (!loaded) {
    return (
      <div className="space-y-4">
        <PageBreadcrumb items={[{ label: '预警管理' }]} />
        <div className="glass-card h-96 flex items-center justify-center">
          <LoadingSpinner label="加载预警数据..." />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <div className="rounded-xl border border-white/10 bg-gradient-to-br from-warning-500/20 to-warning-700/20 p-5">
          ...
          <div className="mt-3 flex items-end justify-between">
            ...
            <span className="text-xs text-white/40">
              {stats.total > 0 ? formatPercent((stats.level1 / stats.total) * 100, 0) : '0%'}
            </span>
          </div>
        </div>
        <div className="rounded-xl border border-white/10 bg-gradient-to-br from-red-500/20 to-red-700/20 p-5">
          ...
          <div className="mt-3 flex items-end justify-between">
            ...
            <span className="text-xs text-white/40">
              {stats.total > 0 ? formatPercent((stats.level2 / stats.total) * 100, 0) : '0%'}
            </span>
          </div>
        </div>
        <div className="rounded-xl border border-white/10 bg-gradient-to-br from-eco-500/20 to-eco-700/20 p-5">
          ...
          <div className="mt-3 flex items-end justify-between">
            ...
            <span className="text-xs text-white/40">
              {stats.total > 0 ? formatPercent((stats.resolved / stats.total) * 100, 0) : '0%'}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-xl border border-white/10 bg-white/5 p-4">
              ...
              {levelOptions.map((opt) => (
                <button
                  ...
                  className={cn(
                    ...
                    level === opt.value && 'bg-white/10 text-white'
                  )}
                >
                  {opt.label}
                </button>
            ...
            <div className="absolute left-0 z-10 mt-2 w-36 overflow-hidden rounded-lg border border-white/10 bg-eco-900 shadow-lg">
              {typeOptions.map((opt) => (
                <button
                  ...
                  className={cn(
                    'block w-full px-4 py-2 text-left text-sm text-white/80 hover:bg-white/10',
                    type === opt.value && 'bg-white/10 text-white'
                  )}
                >
                  {opt.label}
                </button>
            ...
            <div className="absolute left-0 z-10 mt-2 w-36 overflow-hidden rounded-lg border border-white/10 bg-eco-900 shadow-lg">
              {statusOptions.map((opt) => (
                <button
                  ...
                  className={cn(
                    'block w-full px-4 py-2 text-left text-sm text-white/80 hover:bg-white/10',
                    status === opt.value && 'bg-white/10 text-white'
                  )}
                >
                  {opt.label}
                </button>
        ...
        <div className="text-xs text-white/40">共 {filteredAlerts.length} 条记录</div>
      </div>

      {(levelDropdownOpen || typeDropdownOpen || statusDropdownOpen) && (
        <div
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/10 bg-white/5 text-left text-xs uppercase tracking-wider text-white/50">
                <th className="table-header">预警ID</th>
                <th className="table-header">级别</th>
                <th className="table-header">类型</th>
                <th className="table-header">区域</th>
                <th className="table-header">当前值 / 阈值</th>
                <th className="table-header">连续天数</th>
                <th className="table-header">触发时间</th>
              ...
              {filteredAlerts.length === 0 ? (
                ...
                filteredAlerts.map((alert) => {
                  ...
                  return (
                    ...
                    >
                      <td className="table-cell font-mono text-xs text-white/60">{alert.id}</td>
                      ...
                      <td className={cn('table-cell font-medium', typeCfg.color)}>
                        {typeCfg.label}
                      </td>
                      <td className="table-cell text-white/80">{alert.regionName}</td>
                      <td className="table-cell">
